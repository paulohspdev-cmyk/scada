...
def api_create_generator():
    ...
    try:
        ...
        return jsonify({"error":"Código ou porta TCP já cadastrados","detail":str(e)}),409
...
def api_profile(gid):
    ...
    if not obj:
        return jsonify({"error":"não encontrado"}),404
    try:
        return jsonify(load_points(obj["controller_type"]))
...
if __name__=="__main__":
    app.run(host=os.environ.get("RC_BIND","127.0.0.1"), port=int(os.environ.get("RC_WEB_PORT","8088")))
