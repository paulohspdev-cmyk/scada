...
def parse_rtu(frame,unit,count):
    ...
    if frame[0]!=unit: raise ValueError(f"Unit ID inesperado {frame[0]}")
    if frame[1]&0x80: raise ValueError(f"Exceção Modbus {frame[2]}")
    if frame[1]!=3: raise ValueError(f"Função Modbus inesperada {frame[1]}")
    if crc16(frame[:-2])!=struct.unpack("<H",frame[-2:])[0]: raise ValueError("CRC inválido")
    payload=frame[3:-2]
...
async def tcp_response(reader,tid,unit,count):
    ...
    if rt!=tid or proto!=0 or ru!=unit: raise ValueError("MBAP inválido")
    body=await asyncio.wait_for(reader.readexactly(length-1),3)
    if body[0]&0x80: raise ValueError(f"Exceção Modbus {body[1]}")
    if body[0]!=3: raise ValueError("Função Modbus inesperada")
    payload=body[2:]
    return [struct.unpack(">H",payload[i:i+2])[0] for i in range(0,len(payload),2)]

def combine(regs):
    v=0
    for r in regs: v=(v<<16)|r
    return v

async def poll(g,reader,writer):
    points=load_points(g["controller_type"])
    unit=int(g["modbus_unit"]); tid=1
    while not reader.at_eof():
        ...
        db.update_telemetry(g["id"],connected=True,poll_ok=True,values=values,error="")
...
async def reconcile():
    while True:
        ...
        for gid,g in wanted.items():
            ...
            signature=(g["listen_port"],g["transport"],g["modbus_unit"],g["controller_type"])
            if current and current["signature"]==signature:
                continue
            if current:
                current["server"].close(); await current["server"].wait_closed()
            srv=await asyncio.start_server(lambda r,w,gg=g: client(gg,r,w),BIND,int(g["listen_port"]))
        ...
        for gid in list(servers):
            if gid not in wanted:
                servers[gid]["server"].close(); await servers[gid]["server"].wait_closed(); servers.pop(gid)
